    ...
    <div class="container mx-auto px-4 py-8 md:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-800 mb-4 md:mb-0">車両管理</h1>

                <div class="flex flex-col md:flex-row w-full md:w-auto space-y-2 md:space-y-0 md:space-x-2">
                    <x-button purpose="search" href="{{ route('car.create') }}" class="w-full md:w-auto">
                        新規登録
                    </x-button>
                </div>
            </div>
            ...
            <div class="w-full max-w-md mt-10 ">
                <form action="{{ route('car.index') }}" method="GET" class="relative">
                ...
                <table class="w-full">
                    <thead class="bg-sky-200">
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">車両番号</th>
                            <th class="border border-gray-300 px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ナンバープレート</th>
                            <th class="border border-gray-300 px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">車種</th>
                            ...
                            <th class="border border-gray-300 px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">保険終了</th>
                        ...
                        @foreach ($carData as $car)
                            <tr>
                                <td class="border border-gray-300 px-4 py-4">
                                    <div class="text-sm text-gray-900">{{ $car->car_id }}</div>
                                </td>



                                <td class="border border-gray-300 px-4 py-4 ">
                                    <div class="text-sm text-gray-900">{{ $car->number_plate }}</div>
                                </td>
                                <td class="border border-gray-300 px-4 py-4 hidden md:table-cell">
                                    <div class="text-sm text-gray-900">{{ $car->car_type }}</div>
                                </td>
                                <td class="border border-gray-300 px-4 py-4 hidden md:table-cell">

                                    <img src="{{ asset($car->image_path) }}" alt="Car Image" class="w-32 h-28 rounded-lg hover:scale-150 transform transition duration-300">
                                </td>
                                <td class="border border-gray-300 px-4 py-4">
                                    <div class="text-sm text-gray-900">{{ $car->car_insurance_end }}</div>
                                </td>
                                <td class="border border-gray-300 px-4 py-4">
        ...
        {{-- Pagination --}}
        <div class="mt-4">
            {{ $carData->appends(request()->query())->links() }}
        </div>
    </div>
